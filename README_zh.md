...
| **启动自动化** | 可选登录时启动应用，并在应用启动时自动开启永不休眠；两个选项默认关闭。 |
...
- **启动自动化**：macOS 13 及更高版本使用 `SMAppService.mainApp` 管理登录启动。应用自动启动保护时不会修改 `pmset disablesleep`，因此不会意外弹出管理员密码提示；如需合盖防睡眠，请手动开启保护。
